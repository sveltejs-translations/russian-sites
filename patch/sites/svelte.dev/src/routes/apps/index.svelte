...
<script>
	...
	const plApps = (num) => `${num} ${plural(num,['сниппет','сниппета','сниппетов'])}`;
	...
	async function destroy(selected) {
		const confirmed = confirm(
			`Вы уверены, что хотите удалить ${plApps(selected.length)}?`
		);
		if (!confirmed) return;
...
<svelte:head>
	<title>Мои сниппеты • Svelte</title>
</svelte:head>
...
<div class="apps">
	{#if user}
		<header>
			...
			<div class="user">
				...
				<span>
					...
					(<a on:click|preventDefault={logout} href="/auth/logout">выйти</a>)
				</span>
			</div>
		</header>
		...
		<div class="controls">
			{#if selected.length > 0}
				<button class="delete" on:click={() => destroy(selected)} disabled={destroying}>
					<Icon name="delete" />
					Удалить {plApps(selected.length)}
				</button>

				<button on:click={() => (selected = [])}>Очистить выделение</button>
			{:else}
				<form
				...
				{#each gists as gist}
					<li class:selected={selected.includes(gist.id)}>
						<a href={selecting ? undefined : `/repl/${gist.id}`}>
							<h2>{gist.name}</h2>
							<span>обновлен {format(gist.updated_at || gist.created_at)}</span>
						</a>
		...
		{:else}
			<p>У вас нет сниппетов. <a href="/repl">Создайте первый!</a></p>
		{/if}
	{:else}
		<p>
			<a on:click|preventDefault={login} href="/auth/login">Войдите</a>, чтобы увидеть ваши сниппеты.
		</p>
	{/if}
</div>
