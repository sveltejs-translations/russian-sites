...
Vite попытается обработать все импортированные библиотеки и может завершиться ошибкой при обнаружении библиотеки, несовместимой с SSR. Это происходит даже при отключении SSR, если вы также не переопределите значения по умолчанию для `hydrate`, `router` и `prerender.enabled` [как упоминалось в документах](docs#ssr-i-javascript-ssr).
...
Также можено запустить код в `onMount`, если хотите запустить его после того, как компонент был впервые визуализирован в DOM.
