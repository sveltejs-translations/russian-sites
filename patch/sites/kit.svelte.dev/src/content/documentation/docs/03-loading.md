...
Функция `load` получает объект, содержащий четыре свойства — `page`, `fetch`, `session` и `context`. Функция `load` реагирует на параметры, определенные в сигнатуре метода. Если функция написана как принимающая один параметр, она будет повторно запускаться каждый раз при изменении `page`, `session` или `context`. Если параметр объявлен как деструктурированный объект, он будет повторно запущен только при изменении объявленных свойств.
